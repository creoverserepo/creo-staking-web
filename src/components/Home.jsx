...
import { epochFormat, lblFormat, maturityDate, nowTime, numFormat } from "../helpers/TextHelper";
...
const StakeView = () => {
	...
	const submitStake = async () => {
		if (address) {
			setTxLoading(true);
			const tx = await staking.setStake(address, Number(stakeAmount) * Math.pow(10, 18), stakePeriod, 0);
			loadingTransaction(tx);
		} else {
			alert("Please connect your wallet!");
		}
		...
		<div className="body-box">
			...
			<div className="apr-box flex gap-4">
				{stakeOptions.map((val, key) => {
					return (
						val?.apy_d2 > 0 && (
							<button className={"btn-apr " + (key === stakePeriod ? "active" : "")} key={key} onClick={() => setStakePeriod(key)}>
			...
			<div className="info-box">
				<div className="info flex items-center justify-between">
					...
					<div>{lblFormat(String(userStaking?.stakedAmount || 0), 18, " $CREO")}</div>
				</div>
				<div className="info flex items-center justify-between">
					<div>Wallet Balance</div>
					<div>{userBalance ? lblFormat(userBalance, 18, " $CREO") : "..."}</div>
				</div>
				<div className="info flex items-center justify-between">
					...
					<div>{userAllowance ? lblFormat(userAllowance, 18, " $CREO") : "..."}</div>
				</div>
			</div>
			{txLoading && (
				<div className="my-2">
				...
				<>
					<div className="stake-box">
						...
						<div className="relative">
							<input
								...
								onChange={(e) => setStakeAmount(isNaN(e.target.value) ? 0 : e.target.value)}
							/>
					...
					<div className="stat-box grid grid-cols-3">
						<div className="stat">
							...
							<div className="value">{stakeOptions[stakePeriod]?.apy_d2 ? Number(stakeOptions[stakePeriod]?.apy_d2) / 100 + "%" : "..."}</div>
						</div>
						<div className="stat">
							<div className="label">Maturity Date</div>
							<div className="value">{stakeOptions[stakePeriod]?.lockPeriodInDays ? maturityDate(Number(stakeOptions[stakePeriod]?.lockPeriodInDays)) : "..."}</div>
						</div>
						<div className="stat">
							<div className="label">Reward Balance</div>
							<div className="value">{stakeReward ? lblFormat(stakeReward, 18, " $CREO") : "..."}</div>
						</div>
					</div>
					{!address && (
					...
					{address && userAllowance < stakeAmount * Math.pow(10, 18) && stakeAmount * Math.pow(10, 18) >= minStake && (
						<p className="my-4 text-lg text-center font-bold">Increase the allowance amount or to adjust the input to match the allowance amount</p>
					)}
					{address && stakeAmount * Math.pow(10, 18) < minStake && (
						<p className="my-4 text-lg text-center font-bold">Minimum stake amount is {minStake ? lblFormat(minStake, 18, " $CREO") : ".."}</p>
					)}
					{address && (
						<button className="stake-token" onClick={submitStake} disabled={!(address && userAllowance >= stakeAmount * Math.pow(10, 18) && stakeAmount * Math.pow(10, 18) >= minStake)}>
							Stake
						</button>
					)}
				</>
		...
		<div className="history-box">
			<div className="flex items-start justify-between gap-x-4">
				...
				<div className="amount-box">
					...
					<div className="value-token">{userStaking?.stakedAmount ? lblFormat(String(userStaking?.stakedAmount), 18, " CREO") : "..."}</div>
					<div className="value-usd">{tokenPrice && userStaking?.stakedAmount ? "~$" + lblFormat(Number(userStaking?.stakedAmount) * tokenPrice * 1, 18, "") : "..."}</div>
				</div>
			</div>
			{!address && (
				<div className="mt-6">
			...
			{address && !txLoading && !userStakedDetails && <div className="mt-4 text-xl text-center font-bold">Loading data...</div>}
			{address && !txLoading && userStakedDetails && userStakedDetails.length === 0 && <div className="my-12 text-xl text-center">There is no ongoing staking data</div>}
			{address &&
				!txLoading &&
				userStakedDetails &&
							...
							<div className="grid grid-cols-2">
								<div className="info-left-box">
									...
									<div className="detail-box">
										...
										<div className="value">{epochFormat(String(val.stakedAt))}</div>
									</div>
									<div className="detail-box">
										<div className="label">Maturity Time:</div>
										<div className="value">{epochFormat(String(val.endedAt))}</div>
									</div>
								</div>
								<div className="info-right-box">
									<div className="name">{lblFormat(String(val.amount), 18, " CREO")}</div>
									<div className="detail-box">
										<div className="label">Rewards:</div>
										<div className="value">{lblFormat(String(val.reward), 18, " CREO")}</div>
									</div>
									<div className="detail-box">
										<div className="label">Prematurity Penalty:</div>
										<div className="value">{lblFormat(String(val.prematurePenalty), 18, " CREO")}</div>
									</div>
								</div>
							</div>
							<div className="mt-4 flex gap-x-2">
								<button className="btn-withdraw" onClick={() => submitWithdraw(key, val.amount)} disabled={nowTime() >= Number(val.endedAt)}>
									Premature Withdraw
								</button>
								<button className="btn-claim" onClick={() => submitWithdraw(key, 0)} disabled={nowTime() < Number(val.endedAt)}>
									Claim Staking
								</button>
							</div>
						</div>
...
const PastStaking = () => {
	...
	return (
		<div className="history-box">
			...
			<p className="desc">*to view the transaction on blockchain explorer, you can click the Tx Hash.</p>
			{!address && (
				<div className="mt-6">
					<button className="connect-wallet" onClick={open}>
						Connect Your Wallet
			...
			{address && !userUnstakedDetails && <div className="mt-4 text-xl text-center font-bold">Loading data...</div>}
			{address && userUnstakedDetails && userUnstakedDetails.length === 0 && <div className="my-12 text-xl text-center">There is no past staking data</div>}
			{userUnstakedDetails &&
				userUnstakedDetails.map((val, key) => {
					return (
						<div className="item" key={key}>
							<div className="grid grid-cols-2">
								<div className="info-left-box">
									<div className="name">{val?.ev_data?.lockPeriodInDays + " days"}</div>
									<div className="detail-box">
										<div className="label">Staking Time:</div>
										<div className="value">{epochFormat(String(val?.ev_data?.stakedAt))}</div>
									</div>
									<div className="detail-box">
										<div className="label">Maturity Time:</div>
										<div className="value">{epochFormat(String(val?.ev_data?.endedAt))}</div>
									</div>
								</div>
								<div className="info-right-box">
									<div className="name">{lblFormat(String(val?.ev_data?.amount), 18, " CREO")}</div>
									<div className="detail-box">
										<div className="label">Rewards:</div>
										<div className="value">{lblFormat(String(val?.ev_data?.reward), 18, " CREO")}</div>
									</div>
									<div className="detail-box">
										<div className="label">Unstaked Time:</div>
										<div className="value">{epochFormat(String(val?.ev_data?.unstakedAt))}</div>
									</div>
								</div>
							</div>
							<div className="tx-hash-box">
								<div className="label">Tx Hash:</div>
								<a href={`https://bscscan.com/tx/${val.tx_hash}`} target="_blank" rel="noreferrer" className="value">
									{val.tx_hash}
								</a>
							</div>
						</div>
...
function Home() {
	...
	useEffect(() => {
		...
		if (currentSubmenu && listSubmenu.includes(currentSubmenu)) setSubmenu(currentSubmenu);
			...
			<div className={"overlay " + (openExchange ? "active" : "")}>
				<div className="exchange-container">
					<div className="flex items-center justify-between">
						...
						<button className="exchange-close" onClick={() => setOpenExchange(false)}>
							&times;
						</button>
					</div>
					<div className="exchange-list">
						...
						<div className="item">
							<a href="https://www.bitget.com/id/spot/CREOUSDT" target="_blank" rel="noreferrer">
					...
					<div className="w-full lg:flex lg:justify-between lg:items-center lg:gap-8">
						<div className="home-intro-box">
							...
							<div className="py-8 px-4 lg:px-0">
								<button className="btn-primary" onClick={() => setOpenExchange(true)}>
									Buy $CREO
								</button>
							</div>
							{/* <div className="py-8 px-4 lg:px-0">
							...
							<div className="mt-8 home-intro-text">Legacy $CREO staking and farming pools are closed. The new pools are open with Seed Staking Boosters. You can deposit now.</div>
						</div>
								...
								<div className="flex justify-between items-center">
									...
									{menu === "history" && (
										<div className="flex gap-x-4">
											<div className={"cursor-pointer title " + (submenu === "ongoing" ? "active" : "")} onClick={() => chooseSubmenu("ongoing")}>
												Ongoing
											</div>
											<div className={"cursor-pointer title " + (submenu === "ended" ? "active" : "")} onClick={() => chooseSubmenu("ended")}>
												Ended
											</div>
										</div>
									)}
									<select className="option" onChange={(e) => chooseMenu(e.target.value)} value={menu}>
							...
							<div className="left-box">
								...
								<div className="info-box">
									<div className="info flex justify-between items-center">
										...
										<div className="value">{totalValueLocked && tokenPrice ? "$" + numFormat(totalValueLocked * tokenPrice * 1, 18).toFixed(2) : "..."}</div>
									</div>
									<div className="info flex justify-between items-center">
										<div className="label">Total Staked</div>
										<div className="value">{totalStaked ? lblFormat(totalStaked, 18, " CREO") : "..."}</div>
									</div>
									<div className="info flex justify-between items-center">
									...
									<div className="info flex justify-between items-center">
										<div className="label">Percentage CREO staked by circulating supply</div>
							...
							<div className="right-box">
								...
								<div className="stat-container grid grid-cols-3 my-6">
									<div className="stat-left-box text-center">
										<div className="token">{totalStaked ? lblFormat(totalStaked, 18, " CREO") : "..."}</div>
										<div className="value">{totalStaked && tokenPrice ? "~$" + lblFormat(totalStaked * tokenPrice, 18, "") : "..."}</div>
										<div className="label">Total $CREO Staked</div>
									</div>
									<div className="stat-middle-box text-center">
										<div className="token">{totalReward ? lblFormat(totalReward, 18, " CREO") : "..."}</div>
										<div className="value">{totalReward && tokenPrice ? "~$" + lblFormat(totalReward * tokenPrice, 18, "") : "..."}</div>
										<div className="label">Total Pool Rewards</div>
									</div>
									<div className="stat-right-box text-center">
										<div className="token">{totalPending ? lblFormat(totalPending, 18, " CREO") : "..."}</div>
										<div className="value">{totalPending && tokenPrice ? "~$" + lblFormat(totalPending * tokenPrice, 18, "") : "..."}</div>
										...
										{chartLabel && chartData ? (
											<>
												<div className="staking-chart-image">
													<Pie data={{ labels: chartLabel, datasets: [{ data: chartData, backgroundColor: chartBackground, borderWidth: 1 }] }} />
												</div>
												<div className="staking-chart-info ml-4">
													<div className="staking-chart-title">Staking Duration Chart</div>
													{chartLabel.map((_, key) => {
														return (
															<div key={key} className="staking-chart-label" style={{ color: chartBackground[key] }}>
																{"- " + chartLabel[key] + " = " + lblFormat(chartData[key], 18, " CREO")}
															</div>
														);
													})}
												</div>
